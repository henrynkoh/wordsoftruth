...
class PerformanceBenchmarksTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @performance_thresholds = {
      ...
      sermon_crawling: 10.0,
      video_script_generation: 5.0,
      video_processing: 30.0,

      # UI responsiveness requirements
      dashboard_load: 1.0,
      search_results: 0.8,
      statistics_calculation: 1.5,

      # Scalability thresholds
      concurrent_users: 5.0, # 10 concurrent users
      large_dataset_query: 3.0, # 1000+ records
      batch_processing: 60.0, # 50 items
    }
  ...
  test "scalability with increasing dataset sizes" do
    ...
    dataset_sizes.each do |size|
      benchmark_suite("Dataset Size: #{size}") do
        # Create dataset
        measure_operation("Dataset Setup (#{size} items)") do
          size.times do |i|
            create_valid_sermon(
  ...
  test "batch processing performance benchmark" do
    ...
    batch_sizes.each do |batch_size|
      benchmark_suite("Batch Processing: #{batch_size} items") do
        ...
        measure_operation("Batch Submission (#{batch_size} items)") do
        ...
        measure_operation("Batch Video Generation (#{batch_size} items)") do
          stub_video_generation_for_performance
  ...
  test "memory usage patterns during heavy processing" do
    benchmark_suite("Memory Usage Analysis") do
      ...
      memory_after_processing = measure_memory_operation("Sermon Processing") do
        ...
        perform_enqueued_jobs do
          Sermon.all.find_each { |sermon| SermonCrawlingJob.perform_later(sermon.source_url) }
          Sermon.all.find_each { |sermon| VideoProcessingJob.perform_later(sermon.id) }
        end
      end
  ...
  test "database query performance optimization" do
    ...
    benchmark_suite("Database Query Optimization") do
      ...
      measure_operation("Database Setup") do
        100.times do |i|
          sermon = create_valid_sermon(
